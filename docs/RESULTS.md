...
Every number here comes from a command in the repository, and the command that
produced it sits directly above it. Nothing was measured by hand, and nothing is
a screenshot of a run that happened once.

This is the long version. The [README](../README.md) carries the headline result
and the argument; this page is the evidence behind it — the ablation in full, the
forensics, the red team, the failure boundary, and a list of what the test
harness caught while it was being built.

**Where these numbers come from.** Findings are scored against defects the
generator injected at declared rates, so precision and recall are computed
against real labels rather than against the engine's own opinion. Tuning seeds
and evaluation seeds are disjoint and the split is asserted in code. Anything
produced by the offline mock resolver is labelled as such and is never reported
as a model result.
...
The headline version of this is [at the top](../README.md#start-with-the-number-that-argues-against-this-project);
